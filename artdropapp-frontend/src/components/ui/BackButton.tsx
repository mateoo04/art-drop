...
export function BackButton({ to, onClick, label, tone = 'default', className }: BackButtonProps) {
  const colors =
    tone === 'on-image'
      ? 'text-white/90 hover:text-white drop-shadow-lg'
      : 'text-on-surface-variant hover:text-on-surface'
  const baseClassName = `inline-flex items-center group transition-colors ${colors}${
    className ? ` ${className}` : ''
  }`
  const content = (
    <>
